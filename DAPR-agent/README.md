# DAPR Agent - 雨中人绘画情绪探索与艺术表达系统

基于 "Draw-A-Person-in-the-Rain (DAPR)" 绘画活动的交互式情绪探索与艺术表达系统。
...
本项目已验证的预编译 wheel（第三方仓库，含 `cu128` 等较新组合）：
```bash
# CUDA 12.8 + PyTorch 2.9 + Python 3.12 + Linux x86_64
...
> 若 GitHub 下载慢，可用代理：
> ```bash
> pip install https://mirror.ghproxy.com/https://github.com/mjun0812/flash-attention-prebuild-wheels/releases/download/v0.5.4/flash_attn-2.8.3+cu128torch2.9-cp312-cp312-linux_x86_64.whl
> ```

官方 Release 提供部分其他组合的预编译 wheel，可用组合请查看 [官方 Releases](https://github.com/Dao-AILab/flash-attention/releases)，文件名格式为 `flash_attn-{version}+cu{CUDA}torch{PyTorch}cxx11abi{TRUE|FALSE}-cp{py}-cp{py}-linux_x86_64.whl`。
